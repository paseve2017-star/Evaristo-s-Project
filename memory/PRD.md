...
- ON-SITE PROGRESS (2026-09-03): bridge (serial_to_udp.py) running and forwarding on NTPRO PC ✓. Problem reaching separate repeater PC over LAN. Advised single-PC test: install backend on NTPRO PC too (offline bundle), run.bat, open http://localhost:8000; if LED red, switch bridge TARGET to ("127.0.0.1", 4001) for same-PC test. If green on localhost but not across LAN → firewall/subnet issue between PCs.
