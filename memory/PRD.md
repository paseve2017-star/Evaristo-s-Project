...
## Next tasks
1. ON-SITE ISSUE (2026-09-02): repeater PC is OFFLINE (no internet) so pip install failed. SOLVED: prebuilt offline wheel bundles committed at deploy/offline/wheels-py38 (Python 3.8, Win7/2008) and wheels-py312 (Python 3.10-3.12, Win10/11), 15 wheels each ~3.3MB, plus deploy/offline/install_offline.bat; requirements-repeater.txt now minimal (fastapi, uvicorn, websockets, python-dotenv). User re-exports via GitHub to fetch the bundles.
