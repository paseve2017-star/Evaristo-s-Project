...
- User re-exporting via GitHub to fetch offline bundles (incl. pyserial), then on-site install on both PCs (possibly Win7 with Python 3.8 + wheels-py38). Next contact likely = install results: watch for COM port mismatch (bridge serial errors), LED red (firewall/bridge), or heading scale mismatch.
